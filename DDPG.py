...
class DDPG(object):
    def __init__(self, state_dim, action_dim, max_action, larger_critic_approximator, device):
        self.device = device
        ...
        if larger_critic_approximator:
            self.critic = LargerCritic(state_dim, action_dim).to(self.device)
            self.critic_target = LargerCritic(state_dim, action_dim).to(self.device)
