...
if __name__ == "__main__":
    ...
    while total_timesteps < args.max_timesteps:
        ...
        if args.control_buffer_samples:
            if total_timesteps < float(round(args.max_timesteps/5)):
                replay_buffer.add((obs, new_obs, action, reward, done_bool))
            elif total_timesteps > float(round(args.max_timesteps)/3):
                replay_buffer.add((obs, new_obs, action, reward, done_bool))
            elif total_timesteps > total_timesteps > float(round(args.max_timesteps)/1.75):
                replay_buffer.add((obs, new_obs, action, reward, done_bool))
            else:
                pass
